...
public class TaskService {
  ...
  public Page<Task> getAllWithPagination(Pageable page) {
  ...
  public Task create(TaskRequestDTO taskDto) {
    ...
    Project project =
        projectRepository.findById(taskDto.projectId()).orElseThrow(ProjectNotFoundException::new);
  ...
  public void update(Long taskId, TaskRequestDTO taskDto) {
    ...
    if (taskDto.ownerId() == null) {
      ...
      User changedUser =
          userRepository.findById(taskDto.ownerId()).orElseThrow(UserNotFoundException::new);
  ...
  public TaskUploadResponseDTO uploadData(MultipartFile uploadFile) {
    try {
      ...
      return new TaskUploadResponseDTO(
          data.size(), totalSuccess, warnings.size(), errors.size(), warnings, errors);
